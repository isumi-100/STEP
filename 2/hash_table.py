...
class HashTable:
        ...
        while item:
            if item.key == key:
                if prev_item:
                    ...
                    self.buckets[bucket_index] = item.next ## update the head of list, to next
                self.item_count -= 1
                return True
            prev_item = item
            item = item.next
    ...
    def check_size(self):
        # assert (self.bucket_size < 100 )
        self.rehash()

    def rehash(self):
        item_count = 0
        # define new buckets size and new_buckets
        if self.item_count >= self.bucket_size * 0.7:
            new_bucket_size = self.bucket_size * 2 + 1
        elif self.item_count <= self.bucket_size * 0.3 and self.item_count != 0:
            new_bucket_size = self.bucket_size // 2 + (self.bucket_size % 4 + 1) % 2
        else:
            return 0
        ...
        for old_bucket in self.buckets:
            ...
            while item:
                ...
                new_buckets[new_bucket_index] = Item(item.key, item.value, new_buckets[new_bucket_index])
                item = item.next
                item_count += 1
